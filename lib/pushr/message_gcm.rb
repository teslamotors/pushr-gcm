module Pushr
  class MessageGcm < Pushr::Message
    ...
    attr_accessor :registration_ids, :notification_key, :collapse_key, :delay_while_idle, :time_to_live, :data,
                  :restricted_package_name, :dry_run, :notification, :priority
    validates :registration_ids, presence: true
    validate :registration_ids_array
    validate :data_size
    validates :delay_while_idle, :dry_run, inclusion: { in: [true, false] }, allow_blank: true
    validates :time_to_live, numericality: { only_integer: true, greater_than_or_equal_to: 0, less_than_or_equal_to: 2419200 }, allow_blank: true
    validates :priority, inclusion: { in: %w[normal high], message: "'%<value>s' is not a valid priority. Options are: 'normal' OR 'high'" }

    def to_message
      hsh = {}
      hsh['registration_ids'] = registration_ids
      %w[notification_key collapse_key delay_while_idle time_to_live restricted_package_name dry_run priority].each do |variable|
        hsh[variable] = send(variable) if send(variable)
      end
      hsh['data'] = data unless data.nil?
      hsh['notification'] = notification unless notification.nil?
      MultiJson.dump(hsh)
    end

    def to_hash
      hsh = { type: self.class.to_s, app: app, registration_ids: registration_ids, notification_key: notification_key,
              collapse_key: collapse_key, delay_while_idle: delay_while_idle, time_to_live: time_to_live, priority: priority }
      hsh[:data] = data unless data.nil?
      hsh[:notification] = notification unless notification.nil?
